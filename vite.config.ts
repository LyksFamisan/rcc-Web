...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "RCC Website"
  ...
  return {
    name: 'rcc-website-configuration',
    ...
    transformIndexHtml: {
      ...
      handler(html) {
        let result = html
        result = replaceHtmlCommentSlot(result, 'rcc:lang', language)
        result = replaceHtmlCommentSlot(result, 'rcc:title', escapeHtmlText(title))
        result = replaceHtmlCommentSlot(result, 'rcc:head-start', headStart)
        result = replaceHtmlCommentSlot(result, 'rcc:head-end', headEnd)
        result = replaceHtmlCommentSlot(result, 'rcc:body-start', bodyStart)
        result = replaceHtmlCommentSlot(result, 'rcc:body-end', bodyEnd)
...
`,
            ...
            {
              tag: 'a',
              attrs: { class: 'rcc-bypass-link', href: '#root' },
